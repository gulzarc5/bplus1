        ...
        <div class="col-md-4">
          ...
          <div>
            ...
            @else              
                ...
                @if($image->status == 1)
                  <a href="{{ route('admin.product_set_thumb',['product_id'=>encrypt($product->id),'image_id' =>encrypt($image->id) ])}}" class="btn btn-sm btn-success">Set As Main Image</a>

                  <a href="{{ route('admin.product_images_status_update',['product_id'=>encrypt($product->id),'image_id' =>encrypt($image->id),'status'=>encrypt(2) ])}}" class="btn btn-sm btn-warning">Disable</a>
                @else
                  <a href="{{ route('admin.product_images_status_update',['product_id'=>encrypt($product->id),'image_id' =>encrypt($image->id),'status'=>encrypt(1) ])}}" class="btn btn-sm btn-info">Enable</a>
                @endif

              <a href="{{ route('admin.product_images_delete',['product_id'=>encrypt($product->id),'image_id' =>encrypt($image->id)])}}" class="btn btn-sm btn-danger" >Delete</a>
            @endif
          </div>
        </div>
        @endforeach
